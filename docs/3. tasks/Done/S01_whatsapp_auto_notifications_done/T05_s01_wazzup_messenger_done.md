...
- Структура данных `MessageData` с валидацией полей
- Формат Wazzup24 v3 API: `templateId` + `templateValues` (отдельные поля)
- `build_message_text()` — текст сообщения для логирования в DB
- Retry для transient-ошибок (429, 5xx) — по аналогии с kommo.py
- Маскировка телефонов в логах (PII)
- Обработка ошибок: 400, 401, 403, 429, 5xx → `MessengerError`
...
- Переименован `WAZZUP_TEMPLATE_GUID` → `WAZZUP_TEMPLATE_ID` в config.py и .env.example (согласовано с Wazzup24 API field name `templateId`)
- Убраны упоминания Flask из architecture.md и S01 spec (используется только FastAPI)
- Убран `base.py` из file tree в S01 spec (YAGNI)
- Обновлён DoD в S01 spec (BaseMessenger → YAGNI)
...
└── wazzup.py        # WazzupMessenger (lazy singleton)
```
...
```python
from server.messenger import get_messenger, MessageData
...
try:
    ...
    print(f"Текст: {result['message_text']}")
except Exception as e:
    print(f"Ошибка: {e}")
```

### Тест 2: Построение templateValues и message_text
...
# Тест first линия
msg_data = MessageData(line="first", termin_date="25.02.2026")
values = m._build_template_values(msg_data)
text = m.build_message_text(msg_data)
print(f"First values: {values}")
print(f"First text: {text}")
# Ожидается: ["SternMeister", "записи на термин", "25.02.2026"]
# Ожидается: "Здравствуйте. Это SternMeister. Напоминаем о записи на термин в 25.02.2026. Скажите, все в силе?"

# Тест second линия
msg_data = MessageData(line="second", termin_date="26.02.2026")
values = m._build_template_values(msg_data)
text = m.build_message_text(msg_data)
print(f"Second values: {values}")
print(f"Second text: {text}")
# Ожидается: ["SternMeister", "термине", "26.02.2026"]
```

### Тест 3: Валидация MessageData

```python
from server.messenger import MessageData

# Невалидная линия
try:
    MessageData(line="third", termin_date="25.02.2026")
except ValueError as e:
    print(f"Ожидаемая ошибка: {e}")

# Пустая дата
try:
    MessageData(line="first", termin_date="")
except ValueError as e:
    print(f"Ожидаемая ошибка: {e}")
```

### Тест 4: Проверка в WhatsApp

1. Запустить тест отправки на +996501354144
2. Проверить формат сообщения:
   ```
   Здравствуйте. Это SternMeister. Напоминаем о записи на термин в 25.02.2026. Скажите, все в силе?
...
- [x] `WazzupMessenger` реализует `send_message(phone, message_data)`
- [x] `_build_template_values()` корректно формирует параметры WABA-шаблона
- [x] `build_message_text()` возвращает текст сообщения для логирования в DB
- [x] `send_message()` возвращает `message_text` в result dict
- [x] Формат Wazzup24 v3 API: `templateId` + `templateValues` (не legacy `@template:` в text)
- [x] Обработка ошибок: 400, 401, 403, 429, 5xx → `MessengerError`
- [x] Retry для transient-ошибок: 429 и 5xx ретраятся до 3 раз
- [x] `MessageData` валидирует поля (line in {first,second}, termin_date не пуст)
- [x] Телефоны маскируются в логах (PII)
- [x] `requests.Session()` с `User-Agent: SternmeisterBot/1.0`
- [x] `elif` chain в error handling (защита от случайной перестановки)
- [x] Комментарий о non-retry для network errors (POST idempotency risk)
- [x] `WAZZUP_TEMPLATE_ID` (не GUID) — консистентно с API field name
- [ ] Отправка через Wazzup24 API работает: статус 201, messageId возвращается *(требует Docker + реальный API)*
- [ ] Тестовое сообщение приходит на +996501354144 с кнопками *(требует Docker + реальный API)*

---

## Фиксы по код-ревью

1. **`message_text` для DB** — добавлен `build_message_text()` и `message_text` в return dict `send_message()`
2. **`elif` chain** — error handling в `_request_with_retry` переведён на `elif` + комментарий
3. **Network error comment** — добавлен комментарий о non-retry для timeout/connection errors
4. **Rename `template_guid` → `template_id`** — config, .env.example, код
5. **Docs sync** — architecture.md (убран Flask, base.py), S01 spec (DoD, file tree, Flask)
